...
- Name: multipleCheckout.spec.js >> Full purchase flow with 2 products
- Location: tests\multipleCheckout.spec.js:9:1
...
```ts
  ...
  2  | const { doLogin } = require('../utils/loginHelper')
  ...
  6  | const { OrderCompletePage } = require('../pages/OrderCompletePage')
  7  | const { testData } = require('../fixtures/testdata')
  8  | 
  9  | test('Full purchase flow with 2 products', async ({page}) =>{
  10 | 
  11 |     await doLogin(page)
  12 | 
  13 |     const productsPage = new ProductsPage(page)
  14 |     const cartPage = new CartPage(page)
  15 |     const checkoutPage = new CheckoutPage(page)
  16 |     const orderCompletePage = new OrderCompletePage(page)
  17 |     
> 18 |     await productsPage.addBackpackToCart()
     |                        ^ TypeError: productsPage.addBackpackToCart is not a function
  19 |     await productsPage.addBikeLightToCart()
  20 |     await productsPage.goToCart()
  21 |     await cartPage.clickCheckout()
  22 |     await checkoutPage.fillDetails(
  23 |         testData.firstName,
  24 |         testData.lastName,
  25 |         testData.postalCode
  26 |     )
  27 | 
  28 |     await checkoutPage.clickContinue()
  29 |     await checkoutPage.clickFinish()
  30 | 
  31 |     await expect(orderCompletePage.completeHeader).toHaveText('Thank you for your order!')
  32 | 
  33 | })
```