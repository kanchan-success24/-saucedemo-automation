...
- Name: e2e.spec.js >> E2E - Login to Order Complete
- Location: tests\e2e.spec.js:9:1

# Error details

```
TypeError: productsPage.addToCart is not a function
```
...
```ts
  ...
  8  | 
  9  | test('E2E - Login to Order Complete', async ({ page }) => {
  10 | 
  11 |   // Step 1: Login
  12 |   await doLogin(page)
  13 | 
  14 |   // Step 2: Product add karo
  15 |   const productsPage = new ProductsPage(page)
> 16 |   await productsPage.addToCart()
     |                      ^ TypeError: productsPage.addToCart is not a function
  17 |   await productsPage.goToCart()
  18 | 
  19 |   // Step 3: Checkout click
  20 |   const cartPage = new CartPage(page)
  21 |   await cartPage.clickCheckout()
  22 | 
  23 |   // Step 4: Details fill karo
  24 |   const checkoutPage = new CheckoutPage(page)
  25 |   await checkoutPage.fillDetails(
  26 |     testData.firstName,
  27 |     testData.lastName,
  28 |     testData.postalCode
  29 |   )
  30 |   await checkoutPage.clickContinue()
  31 | 
  32 |   // Step 5: Finish
  33 |   await checkoutPage.clickFinish()
  34 | 
  35 |   // Step 6: Assert
  36 |   const orderCompletePage = new OrderCompletePage(page)
  37 |   await expect(orderCompletePage.completeHeader)
  38 |         .toHaveText('Thank you for your order!')
  39 | })
```