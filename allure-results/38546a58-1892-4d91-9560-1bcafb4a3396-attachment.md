...
- Name: cartBadge.spec.js >> Cart badge shows 2 items
- Location: tests\cartBadge.spec.js:5:1
...
```ts
  ...
  4  | 
  5  | test('Cart badge shows 2 items', async ({ page }) => {
  6  | 
  7  |   // Step 1: Login karo
  8  |   await doLogin(page)
  9  | 
  10 |   // Step 2: Page object banao
  11 |   const productsPage = new ProductsPage(page)
  12 | 
  13 |   // Step 3: Pehla product add karo
> 14 |   await productsPage.addToCart()
     |                      ^ TypeError: productsPage.addToCart is not a function
  15 | 
  16 |   // Step 4: Dusra product add karo
  17 |   await productsPage.addBikeLightToCart()
  18 | 
  19 |   // Step 5: Check karo - badge pe "2" hai?
